...
Character.Subscribe("GaitModeChange", function(eChar, _, _) eChar:ComputeSpeed() end)
...
Character.Subscribe("Death", function(eChar, _, _, _, _, pInstigator, _)
    ...
    if not eAttacker or not eAttacker:IsValid() then return end
...
function Character:Disguise(eChar)
    if not IsCharacter(eChar) then return end
...
function Character:UnDisguise()
    if not self:IsDisguised() then return end
...
-- NW: "GM:Character:RequestDisguise"
NW.Receive("GM:Character:RequestDisguise", function(pPlayer, eVictim)
    local eChar = pPlayer:GetControlledCharacter()

    if not IsCharacter(eChar) or not eChar:IsMurderer() or (eChar:GetHealth() <= 0) then return end
    if (eChar:GetCollectedLoot() < GM.Cfg.DisguiseLootRequired) then return end
    if not IsCharacter(eVictim) or (eVictim:GetHealth() > 0) then return end
    if (eChar:GetLocation():DistanceSquared(eVictim:GetLocation()) > 20000) then return end

    local iLastDisguise = eChar:GetValue("last_disguise")
    if iLastDisguise and (CurTime() < (iLastDisguise + GM.Cfg.DisguiseCooldown)) then return end
