...
StaticMesh.Subscribe("ValueChange", function(eSM, sKey, xValue)
    if (sKey ~= "door_open") or not eSM or not eSM:IsValid() then return end
...
local function initCCTV()
    for _, v in ipairs(tCCTVs) do
        ...
        if not v.sceneCapture then goto continue end
...
Package.Subscribe("Load", function()
    ...
    for _, v in ipairs(tRedLightPos) do
        Sound(
            v,
            "package://" ..
            Package.GetName() .. "/Client/resources/sounds/nw_underground/emergency_light_engine_loop.ogg",
            false,
            false,
            SoundType.SFX,
            0.4,
...
-- StaticMesh: "ValueChange"
StaticMesh.Subscribe("ValueChange", function(eSM, sKey, xValue)
    if (sKey ~= "door_id") or not eSM or not eSM:IsValid() then return end
...
StaticMesh.Subscribe("Destroy", function(eSM)
    ...
    if not iDoorID or not tCSDoors[iDoorID] then return end
...
Client.Subscribe("Tick", function(fDelta)
    for _, v in ipairs(tCSDoors) do
        if not v.serverMesh or not v.serverMesh:IsValid() then goto continue end
        if not v.clientMesh or not v.clientMesh:IsValid() then goto continue end
