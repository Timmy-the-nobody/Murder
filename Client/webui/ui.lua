...
GM.WebUI = WebUI("GUI", "file://webui/index.html", WidgetVisibility.Visible)
GM.WebUI:Subscribe("Ready", function()
    GM.WebUI:CallEvent(
        "SetElementInnerHTML",
        "req-loot-bystander",
        "Innocents can loot <b>"..GM.Cfg.BonusRequiredCollectables.." collectable(s)</b> to get a pistol"
    )
    GM.WebUI:CallEvent(
        "SetElementInnerHTML",
        "req-loot-murderer",
        "The murderer can loot <b>"..GM.Cfg.DisguiseLootRequired.." collectable(s)</b> to be able to disguise as a victim"
    )
end)
...
function RebuildKeybinds()
    ...
    if not eChar then
        GM.WebUI:CallEvent(
            "AddBindTooltip",
            Input.GetKeyIcon(Input.GetMappedKeys("Scoreboard")[1], true),
            "Scoreboard"
        )
        GM.WebUI:CallEvent(
            "AddBindTooltip",
            Input.GetKeyIcon(Input.GetMappedKeys("Rules")[1], true),
            "Rules"
        )
        return
    end
    ...
    GM.WebUI:CallEvent(
        "AddBindTooltip",
        Input.GetKeyIcon(Input.GetMappedKeys("Flashlight")[1], true),
        "Flashlight"
    )
    GM.WebUI:CallEvent(
        "AddBindTooltip",
        Input.GetKeyIcon(Input.GetMappedKeys("Taunt")[1], true),
        "Taunt"
    )
    ...
    GM.WebUI:CallEvent(
        "AddBindTooltip",
        Input.GetKeyIcon(Input.GetMappedKeys("Equip/Unequip Weapon")[1], true),
        (bEquiped and "Unequip" or "Equip").." Weapon"
    )

    if bEquiped and eChar:IsMurderer() then
        GM.WebUI:CallEvent(
            "AddBindTooltip",
            Input.GetKeyIcon(Input.GetMappedKeys("Throw Knife")[1], true),
            "Throw Knife"
        )
    end
end
...
Input.Subscribe("KeyBindingChange", RebuildKeybinds)
